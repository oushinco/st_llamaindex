...
model_path = r"/tmp/llama_index/models/llama-2-13b-chat.Q5_K_M.gguf"
...
def process_text_llama_index(text):
    ...
    try:
        ...
        response = llm.complete(the_prompt_str)
        ...
        df_e = pd.DataFrame(json_out['entities'])
        df_r = pd.DataFrame(json_out['relationships'])
    ...
    return json_out, the_prompt_str, the_resp_string, df_e, df_r
    ...
    if st.button("Process the above text"):
        if user_input:
            with st.spinner('Processing your text, please wait...'):
                result_json, the_prompt, the_response, df_e, df_r = process_text_llama_index(user_input)
